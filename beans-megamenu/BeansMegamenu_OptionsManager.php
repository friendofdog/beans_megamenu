...
class BeansMegamenu_OptionsManager {
    ...
    public function settingsPage() {
        if (!current_user_can('manage_options')) {
            wp_die(__('You do not have sufficient permissions to access this page.', 'beans-megamenu'));
        }
        ...
        ?>
        <div class="wrap">
            <h2><?php _e('System Settings', 'beans-megamenu'); ?></h2>
            <table cellspacing="1" cellpadding="2"><tbody>
            <tr><td><?php _e('System', 'beans-megamenu'); ?></td><td><?php echo php_uname(); ?></td></tr>
            <tr><td><?php _e('PHP Version', 'beans-megamenu'); ?></td>
                <td><?php echo phpversion(); ?>
                <?php
                if (version_compare('5.2', phpversion()) > 0) {
                    echo '&nbsp;&nbsp;&nbsp;<span style="background-color: #ffcc00;">';
                    _e('(WARNING: This plugin may not work properly with versions earlier than PHP 5.2)', 'beans-megamenu');
                    echo '</span>';
                }
                ?>
                </td>
            </tr>
            <tr><td><?php _e('MySQL Version', 'beans-megamenu'); ?></td>
                <td><?php echo $this->getMySqlVersion() ?>
                    <?php
                    echo '&nbsp;&nbsp;&nbsp;<span style="background-color: #ffcc00;">';
                    if (version_compare('5.0', $this->getMySqlVersion()) > 0) {
                        _e('(WARNING: This plugin may not work properly with versions earlier than MySQL 5.0)', 'beans-megamenu');
                    }
                    echo '</span>';
                    ?>
                </td>
            ...
            <h2><?php echo $this->getPluginDisplayName(); echo ' '; _e('Settings', 'beans-megamenu'); ?></h2>
            ...
            <?php settings_fields($settingsGroup); ?>
                ...
                <p class="submit">
                    <input type="submit" class="button-primary"
                           value="<?php _e('Save Changes', 'beans-megamenu') ?>"/>
                </p>
            </form>
        </div>
        <?php
    ...
    protected function getOptionValueI18nString($optionValue) {
        switch ($optionValue) {
            case 'true':
                return __('true', 'beans-megamenu');
            case 'false':
                return __('false', 'beans-megamenu');

            case 'Administrator':
                return __('Administrator', 'beans-megamenu');
            case 'Editor':
                return __('Editor', 'beans-megamenu');
            case 'Author':
                return __('Author', 'beans-megamenu');
            case 'Contributor':
                return __('Contributor', 'beans-megamenu');
            case 'Subscriber':
                return __('Subscriber', 'beans-megamenu');
            case 'Anyone':
                return __('Anyone', 'beans-megamenu');
        }
        return $optionValue;
    }
